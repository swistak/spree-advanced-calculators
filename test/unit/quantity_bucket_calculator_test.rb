...
class TestQuantityBucketCalculator < Test::Unit::TestCase
  context Calculator::Advanced do
    setup do
      @order = Factory(:order)
      @calculator = Calculator::QuantityBucket.create(
        :calculable => @order,
    ...
    context "with rates" do
      ...
      should "calculate correctly based on order" do
        Factory(:line_item, :order => @order, :quantity => 95)
        assert_equal(100, @calculator.compute(@order.reload).to_i)

        Factory(:line_item, :order => @order, :quantity => 10)
        @order.reload.update_totals
        assert_equal(200, @calculator.compute(@order).to_i)
      end

      should "calculate based on array of line_items" do
        lis = [
          Factory(:line_item, :order => @order, :quantity => 200),
          Factory(:line_item, :order => @order, :quantity => 10)
        ]
