...
class TestPriceBucketCalculator < Test::Unit::TestCase
  context Calculator::Advanced do
    setup do
      @order = Factory(:order)
      @calculator = Calculator::PriceBucket.create(
        :calculable => @order,
    ...
    context "with rates" do
      ...
      should "calculate correctly based on order" do
        Factory(:line_item, :order => @order, :price => 19.99, :quantity => 1)
        assert_equal(100, @calculator.compute(@order.reload).to_i)

        Factory(:line_item, :order => @order, :price => 19.99, :quantity => 10)
